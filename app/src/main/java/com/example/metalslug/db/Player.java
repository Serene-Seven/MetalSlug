...
public class Player {
    ...
    //此时角色的头部动画必须使用射击的动画帧。
    //角色不能立即发射下一枪一一必须等到leftShootTime为0时，角色才能发射下一枪。
    ...
    public void drawAni(Canvas canvas, Bitmap[] legArr, Bitmap[] headArrFrom, int dir) {
        ...
        //如果处在射击状态，停留次数每次减1，并且采取射击时的图片（只有头部部分）
        if (leftShootTime > 0) {
    ...
    public boolean isHurt(int startX, int endX, int startY, int endY) {
        ...
        int playerEndY = playerStartY + currentHeadBitmap.getHeight() + currentLegBitmap.getHeight();
    ...
    public void move() {
        ...
        } else if (getAction() != Player.ACTION_JUMP_RIGHT && getAction() != Player.ACTION_JUMP_LEFT) {
            if (!isJump()) {
                // 不跳的时候，需要设置动作
    ...
    public void logic() {
        ...
        //如果还没有跳到最高点
        if (!isJumpMax) {
            setAction(getDir() == Player.DIR_RIGHT ? Player.ACTION_JUMP_RIGHT : Player.ACTION_JUMP_LEFT);
            //更新Y坐标
